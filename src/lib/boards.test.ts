...
import { getComponentDefinition, getComponentsForEntity } from './board-components';
import {
...
test('link and pixel boards only offer widgets that make sense for them', () => {
  const types = (entity: 'website' | 'link' | 'pixel') =>
    getComponentsForEntity(entity).map(component => component.type);

  expect(types('link')).not.toContain('EventsChart');
  expect(types('pixel')).not.toContain('Goal');
  expect(types('link')).toContain('WebsiteChart');
  expect(types('website')).toContain('Funnel');
});
