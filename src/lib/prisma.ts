...
const log = debug('ghostwire:prisma');
...
function getSearchParameters(query: string | undefined, filters: Record<string, any>[]) {
  if (!query) return;
...
function getClient() {
  ...
  const adapter = new PrismaPg({ connectionString: url }, { schema: getSchema() ?? undefined });
...
/** The Prisma client, for code that wants it directly (e.g. Better Auth). */
